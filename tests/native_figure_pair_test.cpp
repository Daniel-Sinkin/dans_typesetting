...
auto add_pair(
    Document& document, const std::string_view prefix, const bool reference_second_panel = true
) -> FigurePair&
{
    ...
    return document.blocks().add<FigurePair>(
        ...
        ReferenceId{std::string{prefix} + ":pair"},
        "Joint result",
...
auto run_test() -> void
{
    ...
    add_pair(document, "fig:second", false);
    ...
    expect(
        ...
        "The second pair was not numbered after the first pair"
    );
    expect(
