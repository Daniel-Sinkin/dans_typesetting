      ...
      panel_width_{panel_width}
{
    if (caption.empty())
    {
        throw std::invalid_argument{"A referenceable figure pair must have a caption"};
    }
    ...
    for (const auto& panel : panels_)
    {
        ...
        if (panel_reference.has_value() && panel_reference.value().value() == reference_id_.value())
        {
            throw std::invalid_argument{
...
auto FigurePair::reference_id() const noexcept -> const ReferenceId&
{
    return reference_id_;
}
